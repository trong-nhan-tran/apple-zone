...
import { BannerImageType } from "@/types/schema";
import { ApiResponse } from "@/lib/api-response";

class BannerService {
  private baseUrl = "/api/banner";

  async getAll(categorySlug?: string): Promise<ApiResponse> {
    try {
      const url = categorySlug
        ? `${this.baseUrl}?categorySlug=${categorySlug}`
        : this.baseUrl;
  ...
  async create(data: BannerImageType): Promise<ApiResponse> {
    try {
  ...
  async update(id: string, data: BannerImageType): Promise<ApiResponse> {
    try {
